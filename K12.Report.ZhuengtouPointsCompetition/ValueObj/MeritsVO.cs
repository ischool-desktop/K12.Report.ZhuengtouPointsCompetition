...
namespace K12.Report.ZhuengtouPointsCompetition.ValueObj
{
    ...
    {
        private Dictionary<SchoolYearSemester, List<AutoSummaryRecord>> MeritsBySchoolYear = new Dictionary<SchoolYearSemester, List<AutoSummaryRecord>>();

        public void AddMerit(AutoSummaryRecord rec)
        {
            ...
            if (!MeritsBySchoolYear.ContainsKey(SchoolYearSemester))
                MeritsBySchoolYear.Add(SchoolYearSemester, new List<AutoSummaryRecord>());
            MeritsBySchoolYear[SchoolYearSemester].Add(rec);
        }

        public List<AutoSummaryRecord> GetMeritsBySchoolYear(SchoolYearSemester schoolYearSemester)
        {
            if (MeritsBySchoolYear.ContainsKey(schoolYearSemester))
